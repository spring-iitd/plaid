...
def evaluation_metrics(all_preds, all_labels,max_perturbations,perturbation_type):
    ...
    plt.savefig('./CF/perturb_benign_only/cf_{}{}.png'.format(perturbation_type,max_perturbations), dpi=300)
    # plt.show()
...
def save_image(perturbed_data,n_image,original):
    ...
    plt.savefig('perturbed_benign_images/{}_perturbed_{}.png'.format(original,n_image), dpi=300)
    # plt.show()
...
def Attack_procedure(model, test_model, device, test_loader, perturbation_type, ep, max_perturbations):
    ...
    for data, target in test_loader:
        ...
        if current_target == 0:
            print("Image no:", n_image, "(Benign image)")
            # pack = 1
            ...
            while continue_perturbation and perturbation_count <= max_perturbations:
                ...
                perturbation_count += 1
            save_image(data_denorm, n_image,"benign")
        else:
            print("Image no:", n_image, "(Attack image - skipping perturbation)")
            save_image(data, n_image,"attack")
...
def main():
   ...
   # List of max_perturbations to iterate over
    ...
    for max_perturbations in max_perturbations_list:
        ...
        tnr, mdr, oa_asr, asr = evaluation_metrics(preds, labels,max_perturbations,perturbation_type)
        print("TNR:", tnr)
        print("MDR:", mdr)
        print("OA_ASR:", oa_asr)
        print("ASR:", asr)
