...
def evaluation_metrics(all_preds, all_labels,max_perturbations,perturbation_type):
    ...
    plt.savefig('./CF_Modification_ID_Gradient_densenet161/cf_{}_{}.png'.format(perturbation_type,max_perturbations), dpi=300)
    plt.show()
...
def saving_image(img, name,perturbation_type,max_perturbations):
    ...
    save_image(img, f'./Images_Injection/perturbed_image_{name}.png')
    # img.save(name)
...
def apply_fgsm_and_check( pack, test_model,target,data_grad,data_denorm,ep,perturbation_type,ID,Data,matched_rows,selected_rows_set,bit_pattern):
    ...
    for data, target in test_loader:
        ...
        if current_target == 1:
            ...
            while continue_perturbation and perturbation_count <= max_perturbations:
                ...
                continue_perturbation, pack, final_pred, data_denorm,matched_rows,selected_rows_set = apply_fgsm_and_check(pack, test_model, target, data_grad, perturbed_data, ep, perturbation_type,target_ID,target_Data,matched_rows,selected_rows_set,bit_pattern)
                perturbation_count += 1
