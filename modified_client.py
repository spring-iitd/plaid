...
class Attacker:
    ...
    self.steer_msg = self.db.get_message_by_name('STEERING_CONTROL')
    
    def send_steer_msg(self.torque):
        data = self.steer_msg.encode({'STEER_TORQUE_REQUEST': 1, 'SET_ME_X00': 0, 'SET_ME_X00_2': 0, 'STEER_TORQUE': torque})
        message = can.Message(arbitration_id=self.steer_msg.frame_id, data=data)
        self.can_bus.send(message)
 
    def send_throttle_msg(self.torque):
        self.steer_msg = self.db.get_message_by_name('STEERING_CONTROL')
        data = self.steer_msg.encode({'STEER_TORQUE_REQUEST': 1, 'SET_ME_X00': 0, 'SET_ME_X00_2': 0, 'STEER_TORQUE': torque})
        message = can.Message(arbitration_id=self.steer_msg.frame_id, data=data)
        self.can_bus.send(message)
...
class CAN(object):
    ...
    def send_gear(self, gear):
        if (gear == 0):
            data = self.gear_message.encode({'GEAR_SHIFTER': 4, 'GEAR': 3})
        elif (gear == -1):
    ...
    try:
        ...
        while True:
            ...
            while(1):
                ...
                if 
            if controller.parse_events(client, world, clock, msg):
                return
