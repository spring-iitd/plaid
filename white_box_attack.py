...
def saving_image(img, name,output_path):
    ...
    # save_image(img, f'./DoS_adv_images_old_tst/perturbed_image_{name}.png')
...
def fgsm_attack_valid(image, data_grad,ep,perturbation_type,pack):
    ...
    # save_image(mask, f'./test_mask.png')
...
def apply_injection( pack, test_model,target,data_grad,data_denorm,ep,perturbation_type):
    ...
    # save_image(perturbed_data, f'./test_inj_data.png')
...
def Attack_procedure(model, test_model, device, test_loader, injection_type, modification_type, ep, max_injection_perturbations,output_path):
    pack = 1
    all_preds = []
    all_labels = []
    n_image = 1
    ...
    bit_pattern = "0000010000010000011000" # for matching the packets to modify
...
def main():
    ...
    surr_model_type='wisa'
    test_model_type = 'wisa'


    #Define paths for dataset and model
    test_dataset_dir = '/home/ipali/scratch/code/PLAID-main/Target_dataset_old/test/test_DoS_T_images'
    test_label_file =  '/home/ipali/scratch/code/PLAID-main/Target_dataset_old/test/test_DoS_T_images/DoS_T_test.txt'
    
    surr_model_path = "Trained_Models/dos_spoof_wisa.pth"
    test_model_path = "Trained_Models/dos_spoof_wisa.pth"
    
    output_path = "white_box_dos"

    #folder and filename to save results
    folder = '/home/ipali/scratch/code/PLAID-main/CF_Results/new'
    filename = 'whitebox_dos.png'
   ...
   # List of max_perturbations to iterate over
    ...
    for max_injection_perturbations in max_perturbations_list:
        ...
        preds, labels = Attack_procedure(model, test_model, device, test_loader, injection_type, modification_type, epsilon, max_injection_perturbations,output_path)
        et = time.time()
        print("End time:", et)
        # print("Labels:", labels)
...
##Adversarial attack on Spoofing images##
...
# def saving_image(img, name,folder):
#     # Ensure the folder exists
#     os.makedirs(folder, exist_ok=True)
    
#     # Construct the full path for the output image
#     output_path = os.path.join(folder, f'perturbed_image_{name}.png')
    
#     # Save the image to the specified path
#     save_image(img, output_path)

#     # save_image(img, f'./Gear_adv_images_old_tst/perturbed_image_{name}.png')
...
#             for col in range(1, 1 + ID_len):
#                 pixel_value = perturbed_image[b, :, row, col]
#                 # print(pixel_value[0].item(),type(pixel_value[0].item()))
#                 perturbation_id_bits+=str(int(pixel_value[0].item()))
#                 # dot_product_with_1 = torch.dot(pixel_value, torch.tensor([1.0, 1.0, 1.0], device=image.device))
#                 # dot_product_with_0 = torch.dot(pixel_value, torch.tensor([0.0, 0.0, 0.0], device=image.device))
#                 # if dot_product_with_1 >= dot_product_with_0:
#                 #     # perturbed_image[b, :, row, col] = 1.0  # Set to (256, 256, 256) in range [0, 1]
...
#             # print(perturbation_id_bits,type(perturbation_id_bits))
#             starting_bits = '0' + perturbation_id_bits + middle_bits + perturbation_data_bits
...
#     if perturbation_type == "Gradient":
#         perturbed_image = gradient_perturbation(image, perturbed_image,mask)
#     #needs to be updated for spoofing attack, only updated the gradient_perturbation code.
#     elif perturbation_type == "Targetted": 
#         perturbed_image = fixed_id_data_perturbation(image, perturbed_image,mask,ID,Data)
...
#     target_ID = "00100110000"
#     target_Data = "1010100110111101010101001101100101001110101101110100101011001101"
#     bit_pattern = "0100001111110001000" #for matchign spoofed rows.
#     rgb_pattern = [(0.0, 0.0, 0.0) if bit == '0' else (1.0, 1.0, 1.0) for bit in bit_pattern]
#     pattern_length = len(rgb_pattern)
...
#     3. select model path
#     4. Save images or not and location
#     5. Select the perturbation type
#     6. select the folder to save image in evaluations.
#     '''
...
#     #Define paths for dataset and model
#     test_dataset_dir = '/home/ipali/scratch/code/PLAID-main/Target_dataset_old/test/test_gear_T_images'
#     test_label_file = '/home/ipali/scratch/code/PLAID-main/Target_dataset_old/test/test_gear_T_images/gear_T_test.txt'
    
#     surr_model_path = "Trained_Models/dos_spoof_wisa.pth"
#     test_model_path = "Trained_Models/dos_spoof_wisa.pth"
    
#     output_path = "whitebox_spoof"
#     #folder and filename to save results
#     folder = '/home/ipali/scratch/code/PLAID-main/CF_Results/Gear_Spoofing/new'
#     filename = 'whitebox_spoof.png'
