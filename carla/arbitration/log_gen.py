...
class CAN(object):
    ...
    def send_throttle_brake(self, throttle, brake):
        data = self.throttle_brake_message.encode({'PEDAL_GAS': throttle, 'ENGINE_RPM': 0, 'GAS_PRESSED': 0, 
                                                   'ACC_STATUS': 0, 'BOH_17C': 0, 'BRAKE_SWITCH': 0, 
                                                   'BOH2_17C': 0, 'BRAKE_PRESSED': brake})
...
def game_loop(args):
    ...
    coord_writer = open('./gen_coord.log', 'w')
    timestamp_writer = open('./timestamps_gen.log', 'w')
    control_obj_writer = open('./benign_log.log', 'w')

    gen_control_objs, gen_timestamps, gen_coord = [], [], []

    # subprocess.Popen(
    #     ["../../CarlaUE4.sh", "-carla-server", "-carla-server-ip=10.237.27.193", "-carla-server-port=2000"],
    #     shell=True,
    ...
    try:
        ...
        agent = CustomAgent(world.player)
        ...
        while True:
            ...
            if controller.parse_events():       # to make ESC Key functional
                break
            ...
            can_handler.send_steering(control.steer)
            can_handler.send_throttle_brake(control.throttle, control.brake)
            can_handler.send_gear(control.gear, control.manual_gear_shift)
            gen_control_objs.append(control)
            gen_timestamps.append(current_timestamp)
            gen_coord.append(current_location)
        ...
        for ele in gen_coord:
            coord_writer.write(str(ele) + '\n')
        coord_writer.close()

        for ele in gen_control_objs:
            control_obj_writer.write(str(ele) + '\n')
        control_obj_writer.close()
