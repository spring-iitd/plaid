...
import argparse
gpus= tf.config.experimental.list_physical_devices('GPU')[1]
...
class Model:
    ...
    def read_datainfo(self):
       ...
       "train_unlabel": 0, 
        ...
        self.labels = ['DoS', 'gear', 'RPM', 'Normal']
        # self.labels = ['DoS', 'gear', 'RPM', 'Smart', 'Normal']
        # self.labels = ['DoS', 'Fuzzy', 'gear', 'RPM']
        # data_read = json.load(open('{}/{}/datainfo.txt'.format(self.data_dir, 'TFRecord')))
        # print("--------------------x--------------------")
        # print(data_read)
        ...
        for f in ['{}/Train_0.7_Labeled_0.1/{}/datainfo.txt'.format(self.data_dir, l) for l in self.labels if l != self.unknown_attack]:
    ...
    def construct_data_flow(self):
        train_unlabel_paths = ['{}/Train_0.7_Labeled_0.1/{}/train_unlabel'.format(self.data_dir, l) for l in self.labels]
        # unknown_train_unlabel_path = ['./Data/{}/train_unlabel'.format(self.unknown_attack)]
        train_label_paths = ['{}/Train_0.7_Labeled_0.1/{}/train_label'.format(self.data_dir, l) for l in self.labels if l != self.unknown_attack]
        val_paths = ['{}/Train_0.7_Labeled_0.1/{}/val'.format(self.data_dir, l) for l in self.labels if l != self.unknown_attack]
        ...
        if self.unknown_attack != None:
            val_unknown_path = ['{}/Train_0.7_Labeled_0.1/{}/val'.format(self.data_dir, a) for a in [self.unknown_attack, 'Normal']]
            data_info_unknown_attack = json.load(open('{}/Train_0.7_Labeled_0.1/{}/datainfo.txt'.format(self.data_dir, self.unknown_attack)))
    ...
    def test(self, results_path, unknown_test):
        ...
        saver = tf.compat.v1.train.Saver()
        if unknown_test:
            data_path = ['{}/Train_0.7_Labeled_0.1/{}/'.format(self.data_dir, a) for a in [self.unknown_attack, 'Normal']]
            print("------------------------UNKNOWN ATTACK------------------------", self.unknown_attack)
            ...
            # data_path = ['{}/Train_0.7_Labeled_0.1/{}/'.format(self.data_dir, a) for a in self.labels if a != self.unknown_attack]
            data_path = ['{}/Train_0.7_Labeled_0.1/{}/'.format(self.data_dir, a) for a in ['DoS', 'RPM', 'gear']]
        ...
        with tf.compat.v1.Session() as sess:
            results_path = './results//all/CNN_WGAN_2024-10-01 07:28:29.287804_10_0.0001_64_50_0.5/'
    ...
    def timing(self, x, model_path, num_loop= 100, use_gpu=False):
        ...
        init = tf.compat.v1.global_variables_initializer()
        saver = tf.compat.v1.train.Saver()
        saver.restore(sess, save_path=tf.compat.v1.train.latest_checkpoint(model_path))
        # For warm-up
...
if __name__ == '__main__':
    ...
    if args.is_train:
        ...
        if args.res_path is None:
            ...
            print("xxxxxxxxxxx----------UNKNOWN FALSE----------xxxxxxxxxxx")
            model.test(args.res_path, unknown_test=False)
            # print("xxxxxxxxxxx----------UNKNOWN TRUE----------xxxxxxxxxxx")
